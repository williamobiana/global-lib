...
def call(Map params) {
    log.info(functionName: '-----runPytest - Entry-----')
    log.info params: params

    def packageFile = firstNonNull(params["packageFile"], 'lambda.py')
    def pytestArg = firstNonNull(params["pytestArg"], 'verbose')

    def pytestOutput = sh(script: "pytest ${packageFile} --junitxml=reports/report_${env.BUILD_ID}_${BUILD_TIMESTAMP}.xml ${pytestArg}", returnStdout: true)
    log.info output: "Pytest output: ${pytestOutput}"

    log.info(functionName: '-----runPytest - Exit-----')
}